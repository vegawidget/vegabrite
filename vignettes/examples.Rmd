...
vl_chart() %>%
  ...
  vl_mark_bar() %>% 
  vl_add_axis_config(domainWidth = 1) %>%
  vl_add_view_config(stroke = "transparent")
```
...
vl_chart() %>%
  ...
  vl_encode_color(field = "gender", type = "nominal") %>%
  vl_scale_color(range =  c("#675193", "#ca8861")) %>%
...
bar_chart <- vl_chart() %>%
  vl_mark_bar() %>%
  vl_encode_x("date:O", timeUnit = "month") %>%
  vl_encode_y("precipitation:Q", aggregate = "mean")

point_chart <- vl_chart() %>%
  vl_mark_point() %>%
  vl_encode_x("temp_min:Q", bin = TRUE) %>%
  vl_encode_y("temp_max:Q", bin = TRUE) %>%
...
vl_chart() %>%
  vl_add_data(url = "https://vega.github.io/vega-editor/app/data/cars.json") %>%
  vl_mark_point() %>%
  vl_encode_x(list("repeat" = "column"), "quantitative") %>%
  vl_encode_y(list("repeat" = "row"), "quantitative") %>%
...
vl_chart() %>%
  vl_add_data(url = "https://vega.github.io/vega-editor/app/data/cars.json") %>%
  vl_mark_point() %>%
  vl_encode_x("Horsepower","quantitative") %>%
  vl_encode_y("Miles_per_Gallon", "quantitative") %>%
  vl_encode_color(list("repeat" = "column"), "nominal") %>%
